...
def test_check_fail_fuzzy_match_spaces() -> None:
    """
    It should return unsafe (detected=True) if the token is present but malformed (e.g. spaces).
    Strict integrity check means any deviation is a potential jailbreak or failure.
    """
    canary = Canary()
